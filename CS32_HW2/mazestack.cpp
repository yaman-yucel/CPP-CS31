...
bool pathExists(string maze[], int nRows, int nCols, int sr, int sc, int er, int ec) {
    ...
    //you do not need to check nRows,nCols,sr,sc,er,ec

    stack<Coord> coordStack;

    coordStack.push(Coord(sr, sc)); // push starting point to stack // do not need to check sr sc position
    ...
    while (!coordStack.empty()) // while stack is not empty
    {
        ...
        Coord topCoord = coordStack.top(); // save the first item
        coordStack.pop(); // pop the first item
        ...
        if (inBounds(topCoord.r(), topCoord.c() + 1, nRows, nCols))
        {
            ...
            {
                coordStack.push(Coord(topCoord.r(), topCoord.c() + 1)); // push the coordinate
        ...
        if (inBounds(topCoord.r() - 1, topCoord.c(), nRows, nCols))
        {
            ...
            {
                coordStack.push(Coord(topCoord.r() - 1, topCoord.c())); // push the coordinate
